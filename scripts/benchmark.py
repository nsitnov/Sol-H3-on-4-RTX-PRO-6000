...
SAMPLES = OUT/os.environ.get('H3_SOL_ATTEMPT', 'replication4')
import torch
...
def main():
    ...
    if SAMPLES.exists():
        raise RuntimeError(f'Run directory already exists: {SAMPLES}; choose a new H3_SOL_ATTEMPT')
    cases = json.loads((CASES_PATH).read_text())
    start = time.perf_counter()
    with MiniMaxH3Inference(str(MODEL_ROOT),
            str(ADAPTER_PATH),
        ...
        for case in cases:
            for repeat in range(4):
                ...
                for a in audits:
                    ...
                    assert attention['sparse_calls'] == 144 * attention['requests']
                    assert attention['dense_calls'] == 56 * attention['requests']
                    assert attention['qkv_wire_dtype'] == 'int8_qkv'
                    assert attention['output_wire_dtype'] == 'fp8'
                    assert set(attention['declined']).issubset({'warmup_step','dense_layer'})
                if engine.is_rank_zero:
                    ...
                    record = {'status':'complete','case':case,'repeat':repeat,'warmup':repeat==0,
                        ...
                        'quality':'pending_human_review','world_size':engine.world_size,'steps':4,'seed':42,
                        ...
                        'profile':'SOL/BSA BF16 with INT8 QKV and FP8 output transport; adapted encoder TP',
                        'source_revision':subprocess.check_output(['git','-C',str(SANA_ROOT),
