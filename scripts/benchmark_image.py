...
from common import ROOT, OUT, SANA_ROOT, MODEL_ROOT, ADAPTER_PATH
from image_common import REF_MODEL_ROOT, REF_ADAPTER_PATH, IMAGE_CASES_PATH
TASK = os.environ['H3_SOL_TASK']
MODEL = str(MODEL_ROOT if TASK=='i2v' else REF_MODEL_ROOT)
ADAPTER = str(ADAPTER_PATH if TASK=='i2v' else REF_ADAPTER_PATH)
...
def main():
    ...
    cases = [c for c in json.loads((IMAGE_CASES_PATH).read_text()) if c['sol_task']==TASK]
