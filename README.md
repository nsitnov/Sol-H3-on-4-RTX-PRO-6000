...
The key change is **tensor parallelism for the Qwen3-VL text encoder before the pipeline is moved to CUDA**. The unmodified loading path exceeded each GPU's memory capacity even when four workers were running. Sol-H3 already distributed DiT sequence computation and VAE work; that alone did not distribute the large encoder weights.
...
| Path | Purpose |
|---|---|
| `patches/sol-h3-encoder-tp.patch` | Exact patch used by the successful four-GPU campaign |
...
A stage-based planning model gives approximately **8.2–10.2 seconds for the 5-second case** and **18.8–23.3 seconds for the long 10-second case** on eight comparable cards, under explicit assumptions about compute scaling and communication overhead. Those scenarios correspond to roughly **1.2–1.5× lower latency than our four-GPU run**, not another 5–10× improvement. Poor topology can eliminate the gain or make eight GPUs slower.
